...
enum m_errors m_func(char* a,char* b){
   size_t arg1 = atoi_f(a), arg2 = atoi_f(b);
   if (arg1 == 0 || arg2 == 0){
      return m_error_null_el;
   }
   if (arg2%arg1 == 0){
      return m_succes;
   }
   else{
      return m_no_multiple;
   }
}
                ...
                if (strcmp_f(argv[1], correct_flags[i]) == 0) {
                    ...
                    if (correct_flags[i][1] == 'q') {
                       ...
                       switch(q_func(argv, &result)){
                        ...
                        enum m_errors check = m_func(argv[2],argv[3]);
                        if (check == m_succes) {
                            printf("The first number is a multiple of the second.\n");
                        } 
                        else if(check == m_no_multiple) {
                            printf("The first number is not a multiple of the second\n");
                        }
                        else if (check == m_error_null_el){
                           printf("First or second num = 0\n");
                        }
                        break;
                    }
