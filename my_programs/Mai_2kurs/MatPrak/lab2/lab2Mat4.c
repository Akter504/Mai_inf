...
int is_convex_polygon(size_t count_coord, ...)
{
    ...
    for (int i = 0; i < count_coord-2; i++)
    {
        ...
        if (check != 0) {
            if (sign == 0) {
                // Устанавливаем знак текущего поворота
                sign = (check > 0) ? 1 : -1;
            } else if (sign * check < 0) {
                // Если знак поменялся, то многоугольник не выпуклый
                return 0;
            }
        }
    }
    double lastResult = check_convex(points[count_coord - 2], points[count_coord - 1], points[0]);
    if (sign * lastResult < 0) {
        return 0; // Если знак последнего поворота изменился, то многоугольник не выпуклый
    }
    va_end(args);
    free(points);
    return 1;
...
int main()
{
    if (is_convex_polygon(4, (Points){1.0, 1.0}, (Points){1.0, 0.0}, (Points){0.0, 0.0}, (Points){0.0, 1.0}))
    {
    ...
    int degree = 2;
    double res = 0.0;
    enum errors_poly check = polynomial_value(&res,x,degree,1.0,1.0,1.0); // pow
